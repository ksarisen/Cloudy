...
contract ClientManagerTest /*is ClientManager*/{
    DistributedStorage clientManager;
    bytes20 fileHash1;
    bytes20 fileHash2;
    ...
    function beforeEach() public {
        ...
        clientManager = new DistributedStorage();
    ...
    function testAssignStorageProvider() public {
        ...
        try clientManager.addStorageProvider("127.0.0.1",address(this), 1000000000) {}
        ...
        try clientManager.assignShardToStorageProvider(1,address(this)) {}
            ...
            Assert.ok(false, "Unexpected error in assignShardToStorageProvider");
        }
        ...
        try clientManager.getStorageProviderDetails(address(this)) returns (string memory ip, address walletAddress, uint256 availableStorageSpace, uint256 maximumStorageSize, bool isStoring, uint256[] memory shardIds){
            ...
            bytes32 expectedIPHash = keccak256(bytes(expectedIP));
            bytes32 ipHash = keccak256(bytes(ip));
            Assert.ok(expectedIPHash == ipHash, "IP address returned doesn't match the uploaded test address");
            Assert.ok(expectedIPHash == ipHash, "IP address returned doesn't match the uploaded test address");
        ...
        Assert.ok(true, "assigned shard successfully");
